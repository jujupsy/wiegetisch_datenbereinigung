...
server <- function(input, output, session) {
  ...
  output$processing_status <- renderPrint({

    cat(paste0(processing_states$step, " ", processing_states$step_name))
 ...
 ## Set Range
  ...
  get_paras <- reactive({
      ...
      reload_paras()
      if(input$input_files %in% dir("data_temp"))
       # paras_raw <- reactiveFileReader(100, NULL, 
        #          paste0("./data_temp/", paste0("paras_", input$input_files)), readRDS)
