...
Alle `Parameter` können in der Shiny-Anwendung manuell korrigiert werden, wenn die Datenaufbereitung Fehler aufweist. Außerdem ist es möglich von Hand einzelne Punkte hinzuzufügen, um den tatsächlichen Gewichtsverlauf genau zu rekonstruieren und es können Datenpunkte am Anfang entfernt werden `Remove first # points`.
Um Punkte manuell hinzuzufügen, kann im letzten Plot (unten rechts)  an der gewünschten Stelle mit der Maus geklickt werden. Dadurch wird neben den Schiebereglern ein Feld mit den Koordinaten des Punktes angezeigt und ein Klick auf den Button `Add point manually` fügt den Punkt den Daten hinzu. Der neue Punkt wird dann, mit einem roten Kreis umrandet, im dritten Plot (unten links) angezeigt. Mit `Reset all manual points` können alle hinzugefügten Punkte wieder gelöscht werden. 

### Replizierbarkeit
Damit die Datenaufbereitung nachprüfbar und replizierbar ist, werden alle Parameter für jeden Gewichtsverlauf sowie die manuell gesetzten Punkte in der Datei "para_[Dateiname].rds" gespeichert.
